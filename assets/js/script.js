...
const { container } = ("postcss");
  ...
  const blogs = [
    ...
    { id: 7, title: "Tips Hidup Sehat", image: "https://picsum.photos/id/1018/800/600" },
    { id: 8, title: "Pentingnya Tidur Cukup", image: "https://picsum.photos/id/1025/800/600" },
  ];
...
function displayBlogs(){
    ...
    blogContainer.className="container flex flex-row items-center justify-center space-x-3 px-10 w-3/4 mt-8";

    const leftBlog = document.createElement('a');
    leftBlog.className = "w-2/3 h-[400px] flex flex-row overflow-hidden shadow-lg space-x-3";
    const rightColumn = document.createElement('a');
    rightColumn.className = "w-1/3 flex flex-col h-[400px] space-y-3";
    
    currentBlogs.forEach((blog, index)=>{
        const blogDiv = document.createElement('div');
        blogDiv.className = "relative overflow-hidden shadow-lg hover:scale-105 transition-transform duration-300 hover:rounded-lg cursor-pointer animation-fade-in";
        blogDiv.innerHTML = `
        ...
        if (index < 2){
            blogDiv.classList.add("w-1/2", "h-full");
